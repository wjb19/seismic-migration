function [Summary, finalTrace, srcX, srcY, recX, recY] = ktmWriteTraceBlockConstOffset(file,h,tol,range,opt,sze,imp)

% function [Summary] = ktmWriteTraceBlockConstOffset(file,h,tol,range,opt,sze,imp)
%
% A function to write from seismic input files, SEG-Y format, to *bin files
% (almost) ready for migration etc
%
% INPUT: file(name), offset h, desired trace range and file write mode
...
%
% 
% EXAMPLE: ktmWriteTraceBlockConstOffset('salt.c3na-b.segy',200,10,[1,10000],1,32,1)
...
constants= ktmGetInputMeta(file,2);
disp(['Done in ',num2str(toc),' s']);

tic
disp('Grab Trace MetaData...')
metaData = ktmGetInputMeta(file,3,range);
disp(['Done in ',num2str(toc),' s']);

tic
disp('Grab Trace Data...')
traces = ktmGetInputTrace(file,range);
disp(['Done in ',num2str(toc),' s']);

tic
disp(['Select Traces According to h = ',num2str(h),' +/- ',num2str(tol),'...'])
[metaDataH, tracesH]=ktmGetConstOffTraces(h,tol,0,metaData,traces);
...
Summary=[TRACE_PTS, TIME_STEP, RECORDS, OFFSET, UNITS];

disp('Summary: ');
disp(['TRACE_PTS: ',num2str(TRACE_PTS),' TIME_STEP: ',num2str(TIME_STEP),' RECORDS: ',num2str(RECORDS),' OFFSET: ',num2str(OFFSET)]);
